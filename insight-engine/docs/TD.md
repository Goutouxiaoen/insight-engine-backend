...
> **本文档是**：技术蓝图 —— 用什么技术、为什么选它、工程结构怎么分、关键链路怎么设计。
> **何时看**：技术决策前；不确定"该怎么做"时；面试要讲技术纵深时。
> **不负责**：产品需求（→ `PRD.md`）、接口字段（→ `IF.md`）、表结构（→ `DB.md`）。
...
> ⚠️ **Nacos 例外（2026-09-09 修正）**：Nacos 2.x 会向客户端「自报 ip:port」，客户端据此推导 gRPC 端口（= 主端口 +1000），宿主偏移映射会让客户端去连自报的 `:9848` 而不可达 → **Nacos 宿主端口必须与容器内端口 1:1（8848/9848），不做偏移**（实测修正见 PROGRESS §三 2026-09-09）。
...
| nacos | 8848 / 9848 | **8848 / 9848（1:1）** | ⚠️ 例外：Nacos 2.x 自报地址并推导 gRPC（主端口+1000），偏移映射会致 gRPC 连不上，必须 1:1 |
