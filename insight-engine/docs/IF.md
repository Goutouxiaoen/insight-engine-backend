...
### 3.0 token 载荷口径表（★ 三个签发入口的唯一口径，改前必读）

> 系统里**只有三个地方会签发 token**：登录、刷新、切换空间。它们必须**口径一致**——
> 2026-09-17 的 BE-20260916-01 就是因为"切换空间"另写了一套（按空间裁剪权限）造成的。
> 实现收口：`starter-security` 的 `AuthTokenIssuer`（唯一签发入口）+ `common` 的 `AuthQuerySql`（唯一 SQL 口径）
> + `starter-redis` 的 `TokenSessionCache`（唯一会话落地）。

| 字段 | 登录 `POST /auth/login` | 刷新 `POST /auth/refresh` | 切换空间 `POST /api/v1/workspace/switch` |
|------|------------------------|--------------------------|------------------------------------------|
| `sub`（userId） | 登录用户 | 旧令牌用户 | 旧令牌用户 |
| `tenant_id` | 用户租户 | 同左 | 目标空间所属租户 |
| **`ws_id`（当前空间）** | **成员关系中最早加入的空间**（此刻尚无"当前空间"） | **沿用旧令牌的 `ws_id`**（⚠️ 不得重算默认空间，否则用户被悄悄切回默认空间） | **目标空间** |
| **`roles`** | **用户维度全量**（跨空间聚合，含 `workspace_id` 为空的组织级成员） | 同登录 | **同登录（不按目标空间裁剪）** |
| **`perms`** | **用户维度全量**（按角色展开） | 同登录 | **同登录** |
| 令牌有效期 | access 2h / refresh 7d | 同左（refresh 一次性轮换） | 同左（refresh 一次性轮换） |
...
**语义（2026-09-17 明确，见 §3.0 口径表）**：
- **`ws_id` 沿用旧 refresh token 里的值**（不重算"默认空间"）——否则用户切换空间后一刷新就被悄悄切回默认空间；
- `roles` / `perms` 与登录同口径（用户维度全量）；
- refresh **一次性轮换**：旧 jti 作废、签发新对并覆盖服务端会话；旧 jti 再用即判重放 → 吊销该用户全部会话（`2001`）。
...
**角色授予约束（2026-09-17 新增，防提权）**：本接口允许指定 `roleId`，**"能建号"等价于"能分配角色"**，因此必须校验：

| 校验 | 结果 |
|------|------|
| `roleId` 不存在 | `1001`「角色不存在」（此前会直接落成员关系，产生孤儿数据） |
| 角色不属于当前租户（`ie_role.tenant_id` ∉ {0, 当前租户}） | `1003`「该角色不属于当前租户」 |
| 角色 `scope = ORG/ALL` 且调用者**不是超级管理员** | `1003`「不允许授予该角色：仅超级管理员可授予组织级/平台级角色」 |
| 角色 `scope = WS/SELF`（或调用者是超管） | 允许 |

> 为什么非超管也挡 `ORG`：本接口的门控是 **`member:create`**（空间管理员同样持有），若只挡 `ALL`，
> `ws_admin` 就能通过建号授出 `org_admin`，一步从空间管理员升为组织管理员（详见 PROGRESS §6.3 Y1/Y6）。
> 角色可授予档位规则单点在 `common.RoleGrantPolicy`（UMS 与 workspace 共用，避免两处阈值漂移）。
...
- 不允许删除**当前所处**的工作空间（返回 `1003`，提示先切换）。
  ⚠️ 注意权限顺序：该接口需 `ws:delete`，而 `ws:delete` 属**组织级**权限（`ws_admin` 的 27 条授权里不含），
  故无组织级权限的账号会**先被 `2006` 拦截**，走不到 `1003` 业务校验——这是预期行为（2026-09-17 明确）。
...
- 重签 access token：**只改 `ws_id`（当前上下文 = 目标空间）**；`roles` / `perms` **按「用户」维度取全量、与登录（§3.1）完全同口径**——
  ⚠️ **2026-09-17 修正**：早期实现"按目标空间维度重新展开"会把组织级/平台级能力（`org:*`、`ws:create`、`ws:delete`）筛掉，
  表现为「一切空间就降级」（按钮消失、`ws:delete` 丢失导致 §5.3 的 `1003` 被 `2006` 抢先拦截）；原因是 `ie_member.workspace_id` **可空**（= 组织级成员），
  按空间过滤会整体丢掉这类成员记录。**切换空间 = 换"站在哪个空间"，不是换"我是谁"**（见 `PROGRESS §三` 2026-09-17、`FE-SYNC §2`）；
  ⚠️ 「同一用户在不同空间的权限差异」（如 A 空间能建、B 空间不能建）**不由 token 承载**，属空间维度授权，
  须由服务端按当前 `ws_id` + 成员关系二次判定（TD §7.5 DataScope，`PROGRESS §6.3` 待办）；
- refresh token **一次性轮换**；
...
- **可授予角色受限（2026-09-17 新增，防提权）**：空间内**只能授予 `scope ∈ {WS, SELF}`** 的角色
  （`ie_role.scope`）；`ORG`（org_admin）/`ALL`（super_admin）→ `1003`「不允许授予该角色：超出工作空间管理范围」；
  角色还须归属当前租户（平台内置 `tenant_id=0` 可见），否则 `1003`。
  理由：空间管理员的权能边界就是**这一个空间**，能授予组织级/平台级角色 = 一步完成提权
  （"能授予" 即 "能分配权限"，授权接口本身就是提权接口）；
