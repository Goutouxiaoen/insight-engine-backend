...
> **本文档是**：已完成模块的"实现说明书" —— 实现了哪些功能、每个功能靠哪些类 / 逻辑实现。
> **何时看**：回看某个功能怎么做的；Review 业务逻辑时。
> **不负责**：进度状态（→ `PROGRESS.md`）、接口契约（→ `IF.md`）。
...
- **关键逻辑（五步）**：
  ...
  3. `roles` / `perms` **按「用户」维度取全量（与 UMS 登录同口径）**；
     **2026-09-17 修正**：早期按「目标空间维度」重展开是**错误口径**——`ie_member.workspace_id` 可空（组织级成员）+
     组织级能力（`org:*`/`ws:create`/`ws:delete`）不属于任何空间，按空间过滤必然丢掉它们，表现为「一切空间就降级」
     （按钮消失、`1003` 被 `2006` 抢先拦截）；**切换空间只换上下文（`ws_id`），不换身份（能力）**
...
- **遗留（非本模块）**：「同一用户在不同空间权限不同」属**空间维度授权**，需服务端按当前 `ws_id` 二次判定
  + 前端取「当前空间权限」用于按钮门控（TD §7.5 / PROGRESS §6.3 待办）；token 只承载用户级能力，不承载空间差异
