...
- [x] Docker 网络模型与端口映射（2026-08-25；2026-09-03 修订：**重写类比**——分清「房间内/大堂/楼外」三位置、补「前台如何转接」的 DNAT 机制与两条通路对照表；默认网络→**自定义网络**、补 **compose 项目名前缀**坑、纠偏**服务名 vs 容器名** DNS、补 expose/ports 区别与「不映射端口容器间照样互通」）
...
> 这一类比的**前提是先分清"你站在哪儿"**。很多人觉得含糊，是因为没区分「房间内 / 大堂 / 楼外」三种位置。

**第一步：分清三个位置（关键！）**

把**你的电脑（宿主机）**想象成一栋**公寓楼**。每个容器是楼里**租的一个房间**，服务进程住在房间里。

| 你站在哪儿 | 对应什么 | 举例 |
| --- | --- | --- |
| **① 房间里面** | 容器内的进程 | RabbitMQ 进程、将来打进容器的 ums 服务 |
| **② 楼里的大堂**（不属于任何房间） | **宿主机本身** | 你本机跑的 IDEA、浏览器、psql 客户端 |
| **③ 大楼外面** | 宿主机之外的网络 | 另一台机器、同事的电脑 |

> ⚠️ **最容易想当然的一点**：宿主机（大堂）虽然"在楼里"，但它**不是任何一个房间**。所以大堂里的人想进房间，**和楼外人一样，也必须走前台窗口**。
> 很多人误以为"我人都在这台机器上了，访问容器还不是直接进去"——**不行**。你本机 IDEA 连 RabbitMQ，同样得走 `localhost:5673` 这个前台窗口，不能直接敲 `5672`。

**第二步：三个角色**

1. **房间内部分机号（容器内端口）**：RabbitMQ 这个软件天生规定"我只在房间里的 **5672 号**接电话"（写死在镜像里）。在任何电脑、任何项目跑它，它永远在 5672 号接电话——这就是**容器内端口，永远不改**。

2. **内部电话线 + 查号台（Docker 内部网络 + 内嵌 DNS）**：Docker 给楼里装了一条**内部电话线**，还配了一个自动**查号台**（内嵌 DNS，容器内 `/etc/resolv.conf` 指向 `127.0.0.11`）。房间之间互相联系，**拨的是对方的「名字」（服务名），不是房间号**——因为房间号（容器 IP）每次重建都会变，只有名字不变。

3. **前台窗口（宿主端口映射 = NAT 转发）**：`- "5673:5672"` 的意思是**"在前台开一个 5673 号窗口，凡是打到这个窗口的电话，一律转接到房间里 5672 号分机"**。左边 5673 = 宿主端口（可改），右边 5672 = 容器内端口（不改）。

**第三步：前台到底怎么"找到房间"的（补上原先缺失的那一步）**

外部电话打到 `宿主机IP:5673` → 宿主机的 **iptables DNAT 规则**（Docker 装好后自动维护的）把目标地址改写成 `容器IP:5672` → 送进房间。

容器 IP 每次重建都变，但你**完全不用操心**——这套转发规则是 Docker 守护进程自动维护的，容器一重建它自动改写。

**第四步：两条通路对照（背这张表就够了）**

| 谁 → 找谁 | 走哪条路 | 用什么地址 | 经过前台吗 |
| --- | --- | --- | --- |
| 容器 → 容器 | 内部电话线 | `服务名:容器内端口` = **`rabbitmq:5672`** | ❌ **完全不经过** |
| 宿主机 IDEA → 容器 | 前台窗口 | `localhost:宿主端口` = **`localhost:5673`** | ✅ 必须 |
| 楼外机器 → 容器 | 前台窗口 | `宿主机IP:宿主端口` = `10.x.x.x:5673` | ✅ 必须 |

**第五步：三条结论用类比翻译**

> 📌 **先交代背景，否则这三句话根本没法读**：在写这份 compose **之前**，这台机器上**已经有一个 RabbitMQ 4.2 的容器在跑**（属于之前/别的项目），它已经把**宿主机的 5672 和 15672 端口占用**了。出处：`TD.md` §18.2 端口冲突表 ——"已有 `rabbitmq:4.2` 容器，占用宿主 5672/15672"；`PROGRESS.md` 另记本机已有 `rabbitmq:4.2-management-alpine`、`nacos:v3.1.1`、`minio:latest` 三个镜像**均不复用**。

|  | 别人的 4.2 容器 | 我们的 3.13 容器 |
| --- | --- | --- |
| **房间内端口** | 5672 | 5672（**一样也无所谓**） |
| **前台窗口（宿主端口）** | **5672** | **5673**（一台机器上必须唯一） |
| **容器 IP** | 172.18.0.x | 172.18.0.y（两个不同房间） |

> 🔑 **这张表就是全部答案：房间内端口永远不会冲突（各住各的房间、各有各的网络命名空间），会冲突的只有「前台窗口号」——因为前台窗口在一台机器上必须唯一。**

- **"4.2 占 5672 不影响服务间通信"** → 4.2 占的是**前台 5672 窗口**这一层；而我们的服务间通信走**内部电话线**（`rabbitmq:5672`），根本不碰前台——所以**完全不影响**。
- **"宿主端口改 5673 避开冲突"** → 前台 5672 窗口被 4.2 占了，我们就另开一个"5673 窗口 → 我们自己房间的 5672"。前台窗口**只给大堂（本机 IDE）和楼外（浏览器）用**。
- **"版本锁 3.13 不复用 4.2"** → 干脆不进 4.2 那个房间（那是别人项目的，哪天被删/被改都不知道），自己单独租一个 3.13 房间；而且 4.2 的"电话接口规则"（breaking changes）跟我们客户端版本对不上。

> **一句话记忆：容器之间走内部电话线（叫名字 `rabbitmq` + 房间内分机 5672，绝不过前台）；大堂里的人（本机 IDE）和楼外人想进房间，才走前台窗口（宿主端口 5673）。**
>
> **再补一句防混淆**：两条路**互不相干**——把前台窗口关掉（删掉 `ports` 映射），容器之间照样互通；反过来，容器里写 `rabbitmq:5673` 是错的，那相当于在内部电话线上拨前台窗口号，永远打不通。

### 核心原理

1. **每个容器有独立的网络命名空间**：容器内的进程只监听"容器内端口"，这个端口号由镜像内程序决定（如 RabbitMQ 固定监听 5672/15672），换任何宿主机、换任何项目都不应改它。

2. **容器与宿主机是两套端口体系**：`docker-compose.yml` 里 `ports: - "5673:5672"` 冒号左边是"宿主端口"，右边是"容器内端口"，两者完全独立、可以不同。

   ⚠️ **必须想清楚的一点：宿主机自己，也不在容器的网络里。**
   
   每个容器有独立的**网络命名空间**，容器挂的是 Docker 的虚拟网桥（`insight-net`），而**宿主机本身并不属于这个网桥的一个端口**（宿主机只是网桥的网关/创建者）。因此：
   
   - **宿主机访问容器，同样必须走宿主端口映射**，写 `localhost:5673`，**不能**写 `localhost:5672`——因为宿主 5672 上根本没东西在监听。
   - 这也解释了为什么"本机已有 RabbitMQ 4.2 占了宿主 5672"会让你必须把映射改成 5673：冲突发生在**宿主机这一侧**，跟容器内部毫无关系。

   底层实现：端口映射 = Docker 在宿主机 **iptables 的 NAT 表加了一条 DNAT 规则**（配合 `docker-proxy` 用户态转发做兜底），把 `宿主机IP:5673` 的流量目标地址改写成 `容器IP:5672`。容器重建后 IP 变了，Docker 会自动改写这条规则，所以你不用手工维护。

3. **docker-compose 的内部网络：默认 vs 自定义（本项目是自定义）**。compose 有两种做法：
   
   - **不写 `networks` 段**：compose 自动创建 `<项目名>_default` 网络，所有服务自动加入。
   - **显式自定义**（本项目做法，见 `docker-compose.yml` 末尾）：
     ...
     每个服务再用 `networks: - insight-net` 加入。好处：网络名可控，且未来业务服务（gateway/ums）用**独立的 compose 文件**时，可以复用同一个 `insight-net` 与中间件互通。

   ⚠️ **实操坑（本项目一定会遇到）**：compose 会给**网络名和数据卷名自动加「项目名前缀」**，项目名默认 = compose 文件所在目录名 = `insight-engine`。所以：
   
   - `docker network ls` 里看到的不是 `insight-net`，而是 **`insight-engine_insight-net`**
   - `docker volume ls` 里看到的不是 `pg_data`，而是 **`insight-engine_pg_data`**
   
   跨 compose 文件引用这个网络时，必须写**带前缀的真实名字**，或在 `networks` 里用 `name: insight-net` 显式指定以去掉前缀。

   同一 compose 网络内的服务，彼此能**直接通过「服务名 + 容器内端口」互相访问**（如 `rabbitmq:5672`），走的是内部 bridge 网桥，**根本不经过宿主的任何端口映射**。
...
5. **服务名的 DNS 解析（注意：解析的是「服务名」，不是容器名）**：Docker 守护进程内置了一个 DNS resolver，每个容器内 `/etc/resolv.conf` 都指向 `127.0.0.11`。同一自定义网络内的容器会互相注册「服务名 → 容器 IP」，所以业务服务配置里写 `host: rabbitmq`（**compose 服务名**）+ `port: 5672`（容器内端口）即可。
   
   - 本项目 `docker-compose.yml` 里：**服务名 = `rabbitmq`**，而 `container_name: insight-rabbitmq`。**推荐始终用服务名**，因为 `container_name` 在宿主机上全局唯一（两个 compose 项目会打架），服务名只在项目内唯一、更灵活。
   - **为什么绝不能写死 IP？** 容器每次重建，IP 由 Docker DHCP 重新分配**会变**，只有名字是稳定的。**这就是"服务间通信必须用服务名而不是 IP"的根因。**
   
   ```bash
   # 验证（容器起来后自己敲一遍，比背书有用）
   docker exec insight-rabbitmq cat /etc/resolv.conf   # 看到 nameserver 127.0.0.11
   docker exec insight-redis getent hosts rabbitmq     # 看到解析出的容器 IP
   docker network inspect insight-engine_insight-net  # 看谁挂在这个网络上、各自 IP
   ```

### 为什么「容器内 5672」和「宿主 5672」互不干扰（网络命名空间）

> 疑问：容器里的 RabbitMQ 占了 5672，宿主机的 5672 还能用吗？它俩是同一个端口吗？

**答：是两个完全独立的端口。因为端口不是全局资源，它「隶属于某个网络命名空间」。**

每个网络命名空间都有**自己完整的一套 1~65535 端口表**，彼此看不见：

| 网络命名空间 | 谁的 | 有自己的端口表吗 | 举例 |
| --- | --- | --- | --- |
| 宿主机 | 你这台机器本身 | ✅ 有 | 宿主 5672 是否被占，看宿主机进程与端口映射 |
| 容器 A（我们的 3.13） | RabbitMQ 3.13 | ✅ 有 | 容器内 5672 被 RabbitMQ 进程监听 |
| 容器 B（本机的 4.2） | RabbitMQ 4.2 | ✅ 有 | **容器内也是 5672，照样监听，不冲突** |

所以：

- 容器内的 5672 被 RabbitMQ 占了，**宿主机的 5672 照常可用**（可以给别的进程、也可以给别的容器做映射）。
- 反过来，宿主机 5672 被占了，**也不影响任何容器在内部监听 5672**。
- **可以无限多个容器同时在内部监听 5672**，只要它们的宿主映射端口不同：

```yaml
rabbitmq-a:
  ports: ["5673:5672"]    # 容器内都是 5672
rabbitmq-b:
  ports: ["5674:5672"]    # 容器内都是 5672
rabbitmq-c:
  ports: ["5675:5672"]    # 容器内都是 5672，毫无冲突
```

> 这正是我们 `3.13` 能和本机 `4.2` 共存的原因：**两个容器内部都监听 5672，各在各的命名空间；唯一的差别是宿主窗口号——4.2 用 5672，我们用 5673。**

**那什么时候才会真冲突？**

冲突只发生在**宿主端口这一层**——因为它是所有容器共享的、唯一的资源。两个容器都写 `5672:5672`，第二个就起不来（`port is already allocated`）。

**反例：`network_mode: host`（理解了它，才算真懂隔离）**

```yaml
services:
  rabbitmq:
    network_mode: host    # 不建独立命名空间
```

这样容器**直接共用宿主机的网络栈**，不建自己的命名空间。后果：

- 容器内监听 5672 = **直接占用宿主机 5672**，跟宿主机进程抢端口，会真冲突
- 也不需要（也不能）再写 `ports` 映射了——它本来就直接在宿主网络上
- 换来的是性能略好，代价是**丧失隔离**且与宿主端口强耦合

**自己验证（建议敲一遍）**：

```bash
# 宿主机视角：看宿主 5672 被谁占了
ss -tlnp | grep 5672        # 或 netstat -tlnp | grep 5672

# 容器视角：看容器内 5672 被谁监听（跟上面结果完全是两回事）
docker exec insight-rabbitmq ss -tlnp | grep 5672

# 看宿主机和容器各自的端口表/网络栈，确认是两个世界
docker exec insight-rabbitmq cat /proc/net/tcp
cat /proc/net/tcp
```

### 端口转发全流程（以 Redis `6380 → 6379` 为例）

你在宿主机敲 `redis-cli -h 127.0.0.1 -p 6380`，完整链路：

```text
① 客户端      发出：目标 = 127.0.0.1:6380
      ↓
② 宿主机内核   数据包进入宿主机网络栈，命中 Docker 预先写好的 iptables NAT 规则
              （本机发出的走 OUTPUT 链；外部进来的走 PREROUTING 链）
      ↓
③ DNAT 改写    目标地址被改写成 172.18.0.3:6379（容器IP : 容器内端口）← 这就是"前台转接"
      ↓
④ 网桥转发     经 insight-engine_insight-net 虚拟网桥送进容器
      ↓
⑤ Redis 进程   它在容器内只监听 6379，收到请求并处理
      ↓
⑥ 回程        响应包源地址被 MASQUERADE 改回 宿主机:6380，客户端全程无感
```

三个要点：

- **Redis 进程从头到尾只知道 6379**，它完全不知道外面是 6380。
- 改写发生在**宿主机内核网络层**，对客户端和容器双方都透明。
- 回程由 NAT 自动处理，不需要任何手工配置。

**三种访问场景对照**：

| 场景 | 写的地址 | 走的链路 |
| --- | --- | --- |
| 本机 redis-cli | `127.0.0.1:6380` | 宿主端口 → **DNAT** → 容器 6379 |
| 别的容器（将来的 ums） | `redis:6379` | 内部网络直连，**不走 NAT** |
| 楼外机器 | `服务器IP:6380` | 宿主端口 → **DNAT** → 容器 6379 |

> 本项目用 PostgreSQL 而非 MySQL：`5433 → 5432`（MySQL 同理会是 `3307 → 3306`）。
> **容器内端口（5432 / 3306 / 6379 / 5672）永远不变，变的永远是宿主端口。**

```bash
# 想亲眼看这套 DNAT 规则（云服务器上敲）：
sudo iptables -t nat -L DOCKER -n -v
```

### 两个场景、两条完全不同的路（最易混的地方）

用 RabbitMQ 的 `"5673:5672"` 举例。**同一份 yaml，但"你在 IDEA 里写代码"和"部署到服务器上"，走的是两条完全不同的路。**

**第一步：先搞清楚这四个端口分别属于谁**

| 端口 | 属于谁 | 干什么用 |
| --- | --- | --- |
| **5672** | **RabbitMQ 进程自己监听的**（写在软件里，Docker 管不着） | AMQP 协议 —— 给**程序**连 |
| **15672** | 同上，`-management` 镜像自带的管理界面插件 | HTTP —— 给**浏览器**连 |
| **5673** | **宿主机上你随便定的一个窗口号** | 本机/外部进容器的入口 |
| **15673** | 同上 | 同上 |

> 💡 `rabbitmq:3.13-management` 的 `-management` 后缀 = 镜像**预装了 Web 管理界面插件**，所以才会多出一个 15672。换成不带后缀的 `rabbitmq:3.13` 就只有 5672，没有管理界面。

**第二步：场景 A —— 本地开发（IDEA 里跑 Java，中间件在 Docker）**

你的 Java 进程跑在**宿主机（大堂）**，它要进 RabbitMQ 房间：

```text
IDEA 里的 Java 代码
   ↓  连接 localhost:5673        ← ① 先到【宿主 5673】
   ↓  iptables DNAT 改写
   ↓  变成 172.18.0.x:5672       ← ② 再到【容器 5672】
   ↓
RabbitMQ 进程（它只监听 5672，收到）
```

配置文件写：**`spring.rabbitmq.port=5673`**（本地 profile，host 用 `localhost`）

**第三步：场景 B —— 部署到服务器（业务服务也打进同一个 compose）**

ums 服务和 RabbitMQ **都是容器**，同住一栋楼：

```text
ums 容器
   ↓  连接 rabbitmq:5672         ← 直接到【容器 5672】，没有"先到宿主"这一说
   ↓  走内部网桥 insight-net（DNS 把 rabbitmq 解析成容器 IP）
   ↓
RabbitMQ 容器的 5672 端口
RabbitMQ 进程（收到）
```

配置文件写：**`spring.rabbitmq.port=5672`**（容器 profile，host 用 `rabbitmq`）

> ⚠️ 这个场景下 **5673 完全没被用到**。它只是留给你 SSH 上服务器后用命令行工具排查、或从外面用浏览器看管理界面用的。

**第四步：两张图放一起对比**

| | 本地 IDEA 写代码 | 部署到服务器（容器间） |
| --- | --- | --- |
| Java 进程在哪 | 宿主机（大堂） | 容器（房间） |
| **连接地址** | `localhost:5673` | `rabbitmq:5672` |
| **先到哪个端口** | **先到宿主 5673 → 再 DNAT 到容器 5672** | **直接到容器 5672**（没有"先到宿主"这一步） |
| 走不走前台（NAT） | ✅ 走 | ❌ 不走 |
| 5673 这时有啥用 | 就是你本地调试用的 | 仅供你 SSH 排查 / 浏览器看管理界面 |
| 浏览器管理界面 | `http://localhost:15673` | `http://服务器IP:15673` |

> **一句话收口：`ports` 是给"从外面进容器"开的后门，容器之间从来不走它。**
> 所以同一份 yaml —— 本地走 5673、部署后走 5672，**两个都对，不矛盾**。
...
- **Q2：两个容器（比如我们的 `ums` 服务和 `rabbitmq`）不在同一个 compose 文件里，还能用服务名互访吗？**
  ...
  - 答：默认不行。compose 会为每个项目创建隔离网络，不同 compose 文件（不同项目）之间网络默认隔离，服务名 DNS 不互通。解决两条路：① **共享网络**（推荐）：`docker network create insight-shared` 手动建网，两边 compose 都声明加入同一个外部网络（注意写**带项目名前缀的真实网络名**，见上面第 3 点）；② **走宿主端口**：容器访问宿主机用 `host.docker.internal`（Docker Desktop）或宿主机内网 IP（Linux 服务器），配宿主映射端口。
  - 补充：**同主机跨 compose 用共享网络最干净**；一旦跨主机（多台云服务器），就必须走真实 IP 或 overlay 网络（Swarm/K8s 范畴）。
...
- **Q5（最能戳破误解）：如果一个服务完全不写 `ports` 映射，别的容器还能访问它吗？**
  
  - 答：**能，而且这才是正常做法。** `ports` 只负责"把容器端口暴露给宿主机及外部"，**对容器间通信没有任何贡献**。同一网络内的容器互相访问，靠的是内部网络 + 服务名 DNS，跟宿主机端口映射完全无关。
  - 推论：业务微服务（ums/agent/kb）将来进 compose 时，**可以不映射任何宿主端口**，只加 `expose: ["8081"]` 做文档性声明，网关作为唯一入口对外暴露 8080 即可——更安全、无端口冲突。
  - 顺带区分：`expose` 只是**元数据声明**（写给人和工具看，不产生任何映射）；`ports` 才是**真正的宿主端口映射**。
...
- 注册：`insight-engine-ums = 172.18.0.x:8081`（容器 IP）
  - ⚠️ 网段修正：容器 IP **不是** `172.17.0.x`。`172.17` 是 Docker **默认 bridge** 的网段；本项目用自定义网络 `insight-net`，Docker 会为它**另分配一个网段**（通常 `172.18`/`172.19`…）。用 `docker network inspect insight-engine_insight-net` 看真实网段。
...
- **Q2：容器化后，服务注册到 Nacos 的 IP 是容器内网 IP，会不会访问不到？怎么解决？**
  ...
  - 答：**不能一概而论，要分调用方在哪——这正是最容易记错的地方。**
    
    | 调用方位置 | 注册容器 IP 能用吗 | 原因 |
    | --- | --- | --- |
    | 同 compose 网络的另一个容器 | ✅ **完全可达，没问题** | 容器 IP 在该 Docker 网络内是有效路由 |
    | 宿主机（本地 IDE 直接跑的进程） | ❌ 访问不到 | 宿主机不在该 Docker 网络内，无路由 |
    | 另一台机器的服务 | ❌ 访问不到 | 跨主机，容器内网 IP 不可路由 |
    
    所以**"把所有微服务都打进同一个 compose 网络"时，注册容器 IP 是标准且正确的做法，根本不需要额外配置**。
  - 真正需要干预的，是**混合部署**场景（本项目当前状态：中间件在容器里，微服务在宿主机 IDE 跑）。此时容器内服务注册的是容器 IP，宿主机上的服务拿到这个地址连不上。解决三选一：① `spring.cloud.nacos.discovery.ip` 显式指定注册为宿主机可达 IP；② 指定注册网卡 `network-interface`；③ 容器用 `network_mode: host`（简单但牺牲隔离）。
  - 一句话：**"容器 IP 不可达"不是绝对的，它只在「调用方不在同一 Docker 网络」时成立。**
