...
| model 模型网关 | `/api/v1/model/**` | ⚪ 未开始 | 否（mock） | — | **阶段 6（2026-09-17 判定：有条件进入）**：IF §7.1~§7.8 契约完整、权限码 `model:*` 已在字典；但**开工前先补 DB 缺口**（`ie_model_route` 表缺 → 待裁决"补表 or 路由并入 `ie_model.config`"；`ie_secret` 密钥表缺；厂商/模型种子为空）→ 清单见 `PROGRESS §七 3`。**前端现在可先做模型管理页 UI（照 IF §7 契约 + mock），等 §1 置 ✅ 再切真** |
