...
| model 模型网关 | `/api/v1/model/**` | ⚪ 未开始 | 否（mock） | — | **阶段 6（2026-09-17 判定：有条件进入）**：IF §7.1~§7.8 契约完整、权限码 `model:*` 已在字典；但**开工前先补 DB 缺口**（`ie_secret` 密钥表缺 → 待裁决"新建 or 复用 `ie_sys_config`"；厂商/模型种子为空）→ 清单见 `PROGRESS §七 3`。（2026-09-17 更正：路由策略表**已存在**，名为 `ie_route_policy`，原先"表缺失"的登记系误判，已撤回。）**前端现在可先做模型管理页 UI（照 IF §7 契约 + mock），等 §1 置 ✅ 再切真** |
...
- `[2026-09-17] 答复 BE-20260917-01：空间角色授予限制「未生效」= 运行实例未重启，非契约问题（前端按 IF §5.6 过滤正确，请保持）` → 前端实测 `member/invite` 传 `roleId=1/2` 得 200，原因是**所打实例启动早于修复**（实例 `:7102` 启动 14:28:42；`RoleGrantPolicy.java` 写入 14:35:19、`MemberServiceImpl` 接线 14:36:33、jar 构建 14:37:07、提交 `cf6580c` 14:43）。**member 侧无超管豁免**（`MemberServiceImpl.java:191-197` 只有 scope + 租户两条校验）→ **IF §5.6 无需修改**；超管豁免仅存在于 UMS 建号接口（IF §4.2 已写明"非超管只能选 WS/SELF"）。**实测复现**：含修复的新实例上同一请求 → **`403 / 1003`**「不允许授予该角色：超出工作空间管理范围」；`scripts/smoke-workspace-permission.ps1` 17 项断言 ALL PASS。**请前端在 workspace 服务重启后复测 `roleId=1/2 → 403/1003`、`roleId=3/4/5 → 200`，然后转 🟢**。另：IDEA 的 UMS `:7101` 当前登录返回 `9999`（同日 16:14 实测），请先重启该服务再联调。
