...
| model 模型网关 | `/api/v1/model/**` | 🔵 开发中 | **可联调：厂商 CRUD + 模型目录 CRUD + 聊天补全（非流式 & SSE）+ 路由策略 + 用量查询**（仅 embeddings/rerank 未实现） | **2026-09-17 路由与用量实测**：`scripts/smoke-model-route.ps1` **11 项断言 ALL PASS**（真实千问调用）—— 路由列表 rules 结构化、未知 match 键/strategy → `1001`；**坏主目标 + `fallback=true` 降级成功**（200，resolved=qwen3.7-plus）、`fallback=false` → `502/3004`；用量 org 级 total=9 / **`ws_admin` total=8 且跨空间行不可见**（隔离生效） | **2026-09-17 chat 端到端实测**（真实千问 Token Plan）：非流式 `code=0`+`usage 14/111/125`、`auto`→`qwen3.7-plus`、**SSE 3 个 message 帧 + 1 个 finish（带 usage）**、未知模型 `3001`、**用量落库 3 行**（125/159/298 tokens） | **2026-09-17 种子数据就位 + API Key 已加密接入**：`ie_model_vendor` 1 条（`qwen-token-plan`，Token Plan 专属 `base_url`，`hasApiKey=true`，掩码 `sk-****bvKQ`）+ `ie_model` 10 条 CHAT 模型（默认 `qwen3.7-plus`）；`ie_secret` 存密文（AES-256-GCM/v1，**明文泄露检查 0 行**） | **2026-09-17 模型目录 CRUD 实机冒烟 13/13**（`scripts/smoke-model-catalog.ps1`）：创建厂商（不带 apiKey → `hasApiKey=false`）→ 创建模型 200 → 分页回 `vendorCode/vendorName` 冗余字段且**单价为字符串**（`"0.000800"`，IF §2.5）→ 重复 `(vendorId,code)` → `1001` → 不存在厂商 → `1001`（不产生孤儿）→ 更新（改名称/单价/停用）生效且 `code` 不变 → 删除后不再列出 | **2026-09-17 厂商 CRUD 实机冒烟 12/12**（`scripts/smoke-model-vendor.ps1`，隔离实例 :17101/:17103，云 PG/Redis）：`POST /vendor`（带 apiKey）→ 200；分页回 `maskedHint=sk-****7890` + `hasApiKey=true` 且**响应/分页均不含明文**；`PUT` 不带 apiKey → 密钥不变；带新 apiKey → 掩码轮换；重复 code → `1001`；未知 id → `1004`；`DELETE` → 200 后不再列出。DB 核对：`ie_secret` 为密文（AES-256-GCM/v1），明文泄露 0 行 | `/api/v1/model/vendor/page`(read) / `POST`、`PUT /{id}`、`DELETE /{id}`(write)。**契约要点**：① `apiKey` 明文入参、服务端加密，**读取只回 `maskedHint`（如 `sk-****1a2b`）**，永不下发明文；② `PUT` 的 `apiKey` **留空 = 不改密钥**（前端别用掩码值回填提交）；③ 厂商编码 `code` 唯一且**不可修改**；④ `hasApiKey=false` 表示无鉴权的本地模型（如 Ollama）。⬜ 模型/路由策略/chat completions(SSE) 未实现，**不得声称可联调** | **阶段 6（2026-09-17 判定：有条件进入）**：IF §7.1~§7.8 契约完整、权限码 `model:*` 已在字典；但**开工前先补 DB 缺口**（`ie_secret` 密钥表缺 → 待裁决"新建 or 复用 `ie_sys_config`"；厂商/模型种子为空）→ 清单见 `PROGRESS §七 3`。（2026-09-17 更正：路由策略表**已存在**，名为 `ie_route_policy`，原先"表缺失"的登记系误判，已撤回。）**前端现在可先做模型管理页 UI（照 IF §7 契约 + mock），等 §1 置 ✅ 再切真** |
...
- `[2026-09-17] 模型网关第二批可联调：路由策略（§7.4）+ 用量查询（§7.8）` → **前端可以再做两个页面/模块**：
  1. **路由策略**（`/api/v1/model/route`，读 `model:route:read` / 写 `model:route:write`——实测仅超管与组织管理员持有）：`GET /list`（**返回顺序=匹配顺序**，按 `priority` 升序）、`POST`、`PUT /{id}`、`PUT /{id}/status`。
     - ⚠️ **没有删除接口**（IF §7.4 只定义 list/create/update/status）→ 界面上只能"停用"，**不要放删除按钮**；
     - `rules` 结构：`{strategy:"PRIORITY", fallback:true, rules:[{match:{tenantId?,workspaceId?}, targets:[{modelId}]}]}`；`rules` 与 `targets` **都是有序**（先匹配先生效、首个 target 为主其余为备）；
     - **写入即校验**：未知 `strategy`、未知 `match` 键（含 IF 旧示例里的 `tenantTier`）、空 `targets`、非正整数 `modelId` → `1001` 且 message 说明原因（前端可直接展示该 message）；
     - `priority ≥ 1` 且**数值越小越优先**（默认策略=1，所以"新增的策略要更优先"必须把默认策略停用或批量下移）；
     - `fallback` **仅对非流式生效**（流式不降级，见 IF §7.4）。
  2. **用量分析**（`GET /api/v1/model/usage/page?modelId=&start=&end=&pageNum=&pageSize=`，权限 `model:usage:read`）：
     - `start`/`end` 为 `yyyy-MM-dd`、**闭区间**（含结束日）；返回 `records[]` 含 `modelCode`/`modelName` 冗余字段，`tokens`=total token，**`cost` 为字符串或 `null`**（套餐制无价目 → 显示"未定价"，别渲染成 0）；
     - ⚠️ **可见范围**：`model:usage:read` **空间管理员也有**，后端已按调用者是否持 `org:write` 收敛 → 组织级看全量，**否则只能看到自己当前空间**（前端无需特判，但也**不要**用本接口做"全平台用量总览"给空间管理员看）；
     - 目前只有**明细分页**接口，**没有汇总/图表接口**（如需"近 7 日 token 趋势"，请提工单，后端按需新增）。
- `[2026-09-17] 模型网关核心接口可联调：POST /api/v1/model/chat/completions（非流式 + SSE 流式 + 用量记账）` → **前端对话/调试页可以真机联调了**（权限 `model:list:read`；真实模型 `qwen3.7-plus` 已可用）。
  **契约要点（照 IF §7.5 已定稿的帧格式实现）**：
  1. **请求**：`{model, messages:[{role,content}], stream, temperature, maxTokens}`；`role ∈ system/user/assistant`；
  2. **`model` 可传 `"auto"`**（交给路由策略；路由未实现前 = 第一个启用中的 CHAT 模型）→ **响应里的 `data.model` 是实际使用的模型编码**，建议前端显示出来；
  3. **非流式**（`stream:false`）：返回统一 `Result` 包装，`data = {id, model, choices:[{index, message:{role,content}, finishReason}], usage:{promptTokens,completionTokens,totalTokens}}`；
  4. **流式**（`stream:true`）：`text/event-stream`，**一帧 = `event: <名>\ndata: <JSON>\n\n`**：
     - `message` → `{"model":"...","delta":{"content":"你"}}`（**增量**，前端自行拼接）
     - `heartbeat` → `{"ts":1789634540000}`（**每 15s、不可关闭**；短回答可能一个都不出现，**不要用它判断"是否有数据"**）
     - `finish` → `{"model":"...","finishReason":"stop","usage":{...}}`（**每个流恰好一次**）
     - `error` → `{"code":3003,"message":"模型限流"}`（此时 HTTP 已是 200，**错误码在帧内 code 字段**）
  5. **错误码 3xxx 段已启用**：`3001` 模型不存在/未启用、`3002` 超时、`3003` 上游限流、`3004` 上游失败/厂商未配置、`3005` 密钥问题 —— 前端可按码段做差异化提示；
  6. **`X-Accel-Buffering: no`** 已回写，经网关/nginx 时不会被缓冲（否则会退化成"卡半天刷全部"）。
  **实测证据（2026-09-17，真实调用千问 Token Plan）**：非流式 `code=0`、`content='ok'`、`usage 14/111/125`；`auto` → 解析为 `qwen3.7-plus`；**SSE：3 个 `message` 帧 + 恰好 1 个 `finish`（带 usage）**，拼接内容正确；未知模型 → `3001`；**用量已落库**（`ie_usage_record` 3 行：125/159/298 tokens，`WORKSPACE/1`、`MODEL/ref_id=3`、`cost=NULL`、`trace_id` 齐）。
  **本批仍未实现**：路由策略（§7.4）、Embeddings/Rerank（§7.6/§7.7）、用量查询接口（§7.8，**数据已在累积**，接口未开）→ 前端这几块继续 mock。
- `[2026-09-17] 模型网关第一批接口可联调（厂商 CRUD + 模型目录 CRUD）+ 种子数据就位` → **前端现在可以开工做两个页面**：① **厂商管理**（`/api/v1/model/vendor`：`GET /page`、`POST`、`PUT /{id}`、`DELETE /{id}`，权限 `model:vendor:read` / `model:vendor:write`）；② **模型目录**（`/api/v1/model`：`GET /page`、`POST`、`PUT /{id}`、`DELETE /{id}`，权限 `model:list:read` / `model:list:write`）。
  **可直接用真实数据**：种子已就位——1 条厂商 `qwen-token-plan`（千问AI平台 Token Plan）+ 10 个 CHAT 模型（`qwen3.7-plus` 为默认，另有 `qwen3.7-max`/`qwen3.8-max`/`qwen3.8-flash`/`qwen3.6-flash`/`glm-5.3`/`glm-5.2`/`deepseek-v4-pro`/`deepseek-v4.1-flash`/`deepseek-v4-flash-0731`）；厂商的 API Key **已加密接入**（`hasApiKey=true`，只回掩码 `sk-****bvKQ`）。
  **5 个必须遵守的契约点（避免踩坑）**：
  1. **`apiKey` 永不下发**：读取只回 `maskedHint`（如 `sk-****1a2b`）+ `hasApiKey`；编辑表单**不要用掩码回填提交**——`PUT` 时 `apiKey` **留空 = 不修改密钥**（传掩码串会把密钥写成假值）；
  2. **单价是字符串、且可能为 `null`**：`inputPricePer1k`/`outputPricePer1k` 按 IF §2.5 是 6 位小数字符串（如 `"0.000800"`），**但 Token Plan 是套餐制、暂无价目 → 当前种子值为 `null`**，前端要能显示"未定价"而不是渲染成 0；`contextWindow` 同理可能为空；
  3. **`code` 创建后不可修改**（厂商 `code` 与模型 `code` 都是）：编辑表单里应**置灰**；"换模型"= 新建一条，不要设计成改名；
  4. **`type` 枚举仅 `CHAT` / `EMBEDDING` / `RERANK`**：套餐里的图像（`wan2.7-image*`）与语音（`qwen-audio-3.0-*`）**暂不支持**（扩枚举属契约变更，待裁决）→ 下拉先别做这两类；
  5. **`hasApiKey=false` 是正常态**（无鉴权的本地模型如 Ollama）：前端应引导"去配置密钥"，而不是报错。
  **本批未实现（继续 mock，不得声称可联调）**：模型路由策略（§7.4）、聊天补全 `POST /api/v1/model/chat/completions` 与 SSE 流式（§7.5）、Embeddings/Rerank（§7.6/§7.7）、用量查询（§7.8）。
