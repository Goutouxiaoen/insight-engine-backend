...
| model 模型网关                                | 🔵 进行中 | 40%  | 骨架(7103) + 密钥加密链路 + 厂商 CRUD + 模型目录 CRUD（8 端点） | **2026-09-17 增量 1 + 增量 2 交付并实机冒烟**：① 增量 1（**12/12**）：模块骨架、`ie_secret` 加密链路（`SecretCipher` AES-256-GCM + KEK 环境变量 + `SecretStore`）、厂商 CRUD（apiKey 加密落库、读取只回 `maskedHint`）、网关 `/api/v1/model/**` 路由；② 增量 2（**13/13**）：模型目录 CRUD（`/api/v1/model` page/create/update/delete，厂商外键校验、`(vendorId,code)` 唯一、**单价按 IF §2.5 字符串返回**、厂商信息批量冗余回显、`code` 不可改）；冒烟脚本 `scripts/smoke-model-vendor.ps1` + `scripts/smoke-model-catalog.ps1`；⬜ 待做：路由策略（§7.4）、chat completions(SSE，§7.5)、Embeddings/Rerank（§7.6/§7.7）、用量计量（§7.8） |
| kb 知识库                                    | ⚪ 未开始 | 0%   |                                                |                                        |
| tool 工具市场                                 | ⚪ 未开始 | 0%   |                                                |                                        |
| agent Agent编排                             | ⚪ 未开始 | 0%   |                                                |                                        |
| conv 对话服务                                 | ⚪ 未开始 | 0%   |                                                |                                        |
...
- [2026-09-17] 【**MyBatis-Plus 驼峰映射：数字前不会插下划线**】`Model.inputPricePer1k` 被 MP 映射成列 `input_price_per1k`，而库里是 `input_price_per_1k`（**数字 `1k` 前也有下划线**）→ 创建模型直接 500：`ERROR: column "input_price_per1k" does not exist`（Postgres 还贴心提示 "Perhaps you meant ... input_price_per_1k"）。
  **规则**：MP 的 `map-underscore-to-camel-case` 是"**大写字母前插下划线**"，遇到 `xxx1k` / `xxxV2` 这类"字母数字混排"字段名必然对不上——**列名与 Java 字段名不能一一对应时，用 `@TableField("真实列名")` 显式声明**，别赌自动映射。同样要检查既有实体里是否还有此类命名（如 `per_1k`、`v2`、`2fa`）。


- [2026-09-17] 【**JCE：算法名 ≠ transformation 串**】`SecretCipher` 第一版写 `Cipher.getInstance("AES-256-GCM")` → 运行时 `NoSuchAlgorithmException: Cannot find any provider supporting AES-256-GCM`（创建厂商直接 500）。`Cipher.getInstance(...)` 要的是 **`算法/模式/填充`** 三段式：正确写法 **`AES/GCM/NoPadding`**；"AES-256-GCM" 只是**人类可读的算法名**（继续原样写进 `ie_secret.algo` 列做标识，但**不能**当 transformation 用），密钥长度由 `SecretKeySpec` 决定。已在代码里加注释区分两者，避免后人"改回去"。
  **教训**：加密这种"编译期完全看不出来、只在运行时炸"的代码，**必须冒烟打到真实调用**（本次若不冒烟，会留到接厂商 Key 时才炸）。
- [2026-09-17] 【**PowerShell 参数模式里 `+` 是字面量**（第三次踩）】冒烟脚本里写 `Body 'u2.json' '{"apiKey":"sk-' + $ts + '"}'` → `+` 未被求值，JSON 被拼坏 → 服务端报 `HttpMessageNotReadableException`（表现为"接口 500/1001"，极易误判成后端 bug）。**规则**：函数调用的实参若要拼接字符串，**必须加括号** `Body 'x.json' ('...' + $v + '...')`，或先赋给变量再传。
  **教训**：**脚本报错先怀疑脚本**——本次两条"后端 bug"（`NOAUTH`、`JsonEOFException`）一条是配置缺失、一条是脚本拼串，只有 JCE 那条是真代码缺陷。
...
3. **阶段 6「模型网关」准入（2026-09-17 判定：有条件进入；开工前需 3 项裁决 + 若干必办）**：
   - **【2026-09-17 外部依赖实测：千问 Key 已验证到"账务层"，被欠费拦住】**：开发者提供 DashScope Key 后跑 `scripts/smoke-qwen-connectivity.ps1`，三个接口（非流式 chat / 流式 SSE / embedding）**一致返回** `HTTP 400 {"type":"Arrearage","code":"Arrearage","message":"Access denied, please make sure your account is in good standing"}`（含 `request_id`）。
     **结论（重要，避免后续误判）**：① **`base_url` 与请求格式正确**（请求被正常受理，未报 404/参数错）；② **Key 有效**（无效 Key 会返回 `401 InvalidApiKey`，而这是**账号级**拒绝）；③ 拦路的是**阿里云账户欠费/状态异常**（免费额度用尽或未结清欠费）→ 需在百炼控制台结清/开通，或换其它 OpenAI 兼容厂商（智谱/硅基流动/DeepSeek，**插一条 vendor 记录即可，代码零改动**）或本机 LM Studio/llama.cpp。
     **第二个 Key（`sk-sp-…`）实测 = `401 invalid_api_key`「Incorrect API key provided」** → 与第一个 Key 的 `Arrearage` 形成对照：**同一个 `base_url`、同一套请求体，只有 Key 不同**，故可判定 ① 第一个 Key **有效但账号欠费**；② 第二个 Key **不被 DashScope 认可**（疑似不同平台/不同账号/复制不全）。**待开发者确认 Key 来源平台**后再定 base_url（若来自第三方聚合平台，`base_url` 必须换成该平台的 OpenAI 兼容入口）。
     **纪律**：Key 全程只经进程环境变量（**未落盘、未入库、输出只有掩码**），用后已从会话清除；**并已提醒开发者该 Key 因出现在对话记录中而应视为已暴露、建议轮换**。**阶段 6 不因此阻塞**（后续可用 mock/本地模型验证链路）。
   - **【2026-09-17 增量 1 已交付（进入编码）】**：模型网关**模块骨架 + 密钥加密链路 + 厂商 CRUD（4 端点）**已完成并**实机冒烟 12/12 ALL PASS**（`scripts/smoke-model-vendor.ps1`）——覆盖"创建带 apiKey → 响应/分页**均不含明文**、掩码 `sk-****xxxx`、带 `hasApiKey`、**不带 apiKey 更新密钥不变**、带新 apiKey 掩码轮换且**复用同一 `ie_secret` 行**、重复 code→`1001`、未知 id→`1004`、逻辑删除后不再列出"；DB 侧核对：`ie_secret` 落库为密文（`cipher_text`/`iv`/`algo=AES-256-GCM`/`kek_version=v1`/`masked_hint`），**明文泄露检查 = 0 行**；网关新增 `/api/v1/model/**` → `lb://insight-engine-model`。下一步：模型 CRUD、路由策略、`chat/completions`（SSE）、Embeddings/Rerank、用量计量。
...
- 日期：2026-09-17（阶段 6 模型网关 · 增量 2 + Key 连通性诊断）
- 内容：**① 千问 Key 连通性实测（结论：Key 有效，卡在账号欠费）**：开发者提供的 DashScope Key 跑 `smoke-qwen-connectivity.ps1`，三个接口一致返回 `HTTP 400 {"type":"Arrearage","code":"Arrearage"}` → **`base_url`/请求格式正确、Key 有效**（无效 Key 会 401 `InvalidApiKey`，这是**账号级**拒绝），拦路的是**阿里云账户欠费/状态异常**（结清/开通，或换厂商/本地模型，代码零改动）。Key 全程只经进程环境变量、未落盘、输出仅掩码，已提醒轮换。② **增量 2：模型目录 CRUD（8 端点累计）**：`Model` 实体 / Mapper / 3 个请求 DTO / `ModelVO`（**单价按 IF §2.5 用 `ToStringSerializer` 出字符串**）/ `ModelService(+Impl)` / `ModelController`；厂商外键校验、`(vendorId,code)` 唯一、`code` 不可改、厂商信息**批量 IN 冗余回显**（避免 N+1）；IF §7.3 补齐请求体与业务规则。③ **验证**：`mvn package` **BUILD SUCCESS**；`scripts/smoke-model-catalog.ps1` **13 项断言 ALL PASS**（含单价字符串断言、孤儿防护、`code` 不可变、删除后不列出）。④ **踩坑一条（真 bug，冒烟抓到）**：MyBatis-Plus 驼峰映射**不会在数字前插下划线**（`inputPricePer1k` → `input_price_per1k`，库里是 `input_price_per_1k`）→ 创建模型 500；已用 `@TableField("真实列名")` 修复并立规则（见 §四）。⑤ **环境**：探针数据已清（`ie_model`/`ie_model_vendor` 归零）、隔离实例已停、临时文件与 KEK 已清、IDEA 三服务未动。

- 日期：2026-09-17（阶段 6 模型网关 · 增量 1）
- 内容：**进入模型网关，交付骨架 + 密钥加密链路 + 厂商 CRUD** —— ① **BE-20260917-01 结案澄清**：开发者重启后复测 **`roleId=1` 仍 200**，实测进程表发现**只重启了 UMS**（PID 10340→37232@16:17），**workspace `:7102` 仍是 14:28:42 的旧进程**（gateway `:7000` 同）→ 结论再确认：「不是代码问题，是 workspace 未重启」；IDEA 输出目录即 `target/classes`（`MemberServiceImpl.class` 14:37:07 已含修复）→ **重启 workspace 即生效**，已同步 `FE-SYNC §2`。② **阶段 6 增量 1**（14 个新文件）：`insight-engine-model` 骨架（pom/`ModelApplication`/`application.yml`+local 模板/`OpenApiConfig`/`ModelConstants`）、**密钥链路**（`SecretCipher` AES-256-GCM + KEK 只从环境变量/本机 local.yml 读 + `SecretStore` 加密落库/掩码）、**厂商 CRUD 4 端点**（`/api/v1/model/vendor` page|create|update|delete，apiKey 明文入参→密文落 `ie_secret`、读取只回 `maskedHint`、**不带 apiKey 更新不改密钥**、更新**复用同一密钥行**）、网关新增 `/api/v1/model/**` → `lb://insight-engine-model`；新增 `.env.example` 的 `INSIGHT_SECRET_KEK`（铁律 5 三件事齐备）。③ **验证**：`mvn -DskipTests package` **BUILD SUCCESS**；`scripts/smoke-model-vendor.ps1` **12 项断言 ALL PASS**；DB 核对密文/IV/算法/KEK 版本/掩码 + **明文泄露 0 行**；测试数据已清理、隔离实例已停、IDEA 三服务未动。④ **踩坑两条**（已入 §四）：JCE transformation 必须写 `AES/GCM/NoPadding`（写算法名运行时才炸，冒烟当场抓到）；PowerShell 参数模式 `+` 是字面量（脚本坑，第三次踩，已立规则）。⑤ **下一步**：模型 CRUD → 路由策略 → `chat/completions`(SSE) → Embeddings/Rerank → 用量计量；DashScope Key 连通性自测（`scripts/smoke-qwen-connectivity.ps1`）待开发者提供 Key 后执行。
