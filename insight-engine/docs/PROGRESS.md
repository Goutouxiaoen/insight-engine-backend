...
- [2026-09-16] 坑：**gateway 关闭时抛 `ERR_NACOS_DEREGISTER ... Client not connected, current status:STARTING` → 根因不是网络，而是 `NACOS_ADDR` 没注入**：gateway `application.yml` 的 Nacos 地址写成 `${NACOS_ADDR:127.0.0.1:8848}`（**有默认值 → 不 fail-fast**），IDEA 里没配 profile/环境变量时静默回落本机 127.0.0.1 → 启动期刷 `Server check fail ... 127.0.0.1:9848`、关闭期抛上面那条（**报错出现在 shutdown 线程，属症状非根因**）、`lb://` 路由不可用。**与 UMS/workspace 的差异**：它们的 `datasource`/`redis` 占位符无默认值 → 缺变量直接启动失败（报错显眼）；gateway 这类"有默认值"的配置则**静默降级**，更难发现。→ 规避：① 三服务均补 `application-local.yml` 的 Nacos 地址（UMS/workspace 原先只覆盖 DB/Redis，**Nacos 段是漏的**，故其 local 模式同样刷 Nacos 报错）；② 固化共享运行配置 `.run/*.run.xml`（含 `GatewayApplication (local)`）；③ gateway 设 `register-enabled: false`（纯消费方）。**实测**：修复后启动 1.84s、`127.0.0.1` 相关报错 **0 条**、关闭时 `ERR_NACOS_DEREGISTER` **0 条**（见 §六 6.4 `lb://` 复验）
...
- [x] 🟠 **P1-1 `NACOS_ADDR` 注入路径不可追溯** —— **2026-09-16 已修**：① UMS / workspace 的 `application-local.yml` 补上 `spring.cloud.nacos.discovery|config.server-addr`（原先只覆盖 DB/Redis，**Nacos 段漏配**，故 local 模式一样刷 Nacos 报错）；② gateway 新建 `application-local.yml` + `application-local.example.yml`；③ 三服务共享运行配置固化入库 `insight-engine/.run/`（`UmsApplication (local)` / `WorkspaceApplication (local)` / `GatewayApplication (local)`，只写 profile 名，不含口令与 IP）→ IDEA 启动不再依赖 shell 临时变量。**实测**：三服务 local 启动，Nacos 报错 0 条、`lb://` 双服务转发 200
...
1. **workspace 收尾（阶段 5 主线，最优先）**：代码已实现并实机冒烟 28/28 → ① 代码**已提交并推送** `feature/workspace`（GitHub Flow，**待 PR 合并**）；② 通知前端按 FE-SYNC §1 切真联调（`workspace,member` 移出 mock 清单；同时改 `POST /api/v1/member` → `/api/v1/member/invite`），闭环 BE-20260908-02；③ ✅ Nacos `lb://` 转发已复验通过（2026-09-16，安全组放行本机 IP 后，UMS+workspace 双服务，见 §6.4）
