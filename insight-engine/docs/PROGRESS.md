...
| model 模型网关                                | 🔵 进行中 | 20%  | 骨架(7103) + 密钥加密链路(AES-256-GCM) + 厂商 CRUD 4 端点 | **2026-09-17 增量 1 交付并实机冒烟 12/12**：模块骨架（pom/启动类/application.yml+local 模板/OpenApiConfig/常量）、`ie_secret` 加密链路（`SecretCipher` AES-256-GCM + KEK 环境变量 + `SecretStore`）、厂商 CRUD（`/api/v1/model/vendor` page/create/update/delete，apiKey 加密落库、读取只回 `maskedHint`）、网关新增 `/api/v1/model/**` 路由；⬜ 待做：模型 CRUD、路由策略、chat completions(SSE)、Embeddings/Rerank、用量计量 |
| kb 知识库                                    | ⚪ 未开始 | 0%   |                                                |                                        |
| tool 工具市场                                 | ⚪ 未开始 | 0%   |                                                |                                        |
| agent Agent编排                             | ⚪ 未开始 | 0%   |                                                |                                        |
| conv 对话服务                                 | ⚪ 未开始 | 0%   |                                                |                                        |
