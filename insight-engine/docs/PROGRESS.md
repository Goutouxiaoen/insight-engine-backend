...
3. **阶段 6「模型网关」准入（2026-09-17 判定：有条件进入，开工前先办 5 件事）**：
   ...
   - ⬜ **必须补（DB/契约缺口，按铁律 2/6 先裁决再动手）**：
     1. **`ie_model_route` 表缺失**：IF §7.4 有 5 个路由策略接口（含 `rules` JSONB 结构），但 `init.sql` 与 `DB.md` **都没有该表** → 二选一裁决：**(a)** 补表（DB.md + init.sql + 云端执行）；**(b)** MVP 不落表，路由策略并入 `ie_model.config` JSONB（则 IF §7.4 需改写成"模型级 fallback"）。**未裁决前不得照 IF 直接建表**；
     2. **`ie_secret` 密钥表缺失**：`ie_model_vendor.api_key_secret_id` 已引用它（`init.sql:1150` 注释自认"阶段 3/8 落地时补充"），而 TD §16.1 要求「模型 API Key：**AES-256-GCM 加密存储，KEK 放环境变量/Secret**」→ 本章必须落地表 + 加密组件（并同步 `.env.example` 与文档变量名，铁律 5 三件事）；
     ...
     5. **需开发者提供的输入**：① 通义 DashScope **API Key**（放 `.env` 或密钥表，**不入库明文**）；② 本机/云端是否有 **Ollama** 可跑（决定"离线兜底模型"是否可实测）；
...
- 内容：**workspace 阶段收尾 + 「模型网关」准入判定** —— ① **收尾三件套**：全量 `mvn -DskipTests package` **BUILD SUCCESS**；`scripts/smoke-workspace-permission.ps1` **17 项断言 ALL PASS** + 跨租户实测（`DELETE`/`my-permissions` → 404/1004）；文档同步（IF §4.2/§5.6/§5.7、TD §6.1/§7.5、PROGRESS §二/§三/§6.3/§七/§八、FE-SYNC §1/§2、FEATURES 4.1、LEARNING）；提交 `15d7504` + `cf6580c` 已推送 `feature/workspace`（领先 master **20 commits**）。② **补齐铁律 1 漏掉的双写**：`BE-20260916-01` 前端侧仍是 `🟡 待后端答复`（`FE-SYNC` 早已答复），本轮回填「后端答复 + 证据 + 状态 ✅」并提示前端核对后改 `🟢`；同时把 `my-permissions` 门控建议与角色授予收紧一并写入该条（避免前端只看 FE-SYNC 漏掉）。③ **准入判定（结论：有条件进入阶段 6）**：契约/权限码/表（`ie_model_vendor`/`ie_model`）/starter 能力/SSE 心跳契约均就绪；**但发现 4 项 DB 或契约缺口必须先办**——**(a)** `ie_model_route` 表在 `init.sql` 与 `DB.md` 中**均不存在**（而 IF §7.4 有 5 个路由策略接口）→ 需裁决"补表 or 路由并入 `ie_model.config`"；**(b)** `ie_secret` 密钥表缺失（`api_key_secret_id` 已引用它、TD §16.1 要求 AES-256-GCM），本章必须落地；**(c)** 厂商/模型种子为空（云端实测 0 条）+ 需真实 DashScope Key（`.env`/密钥表，不入库明文）；**(d)** `ie_usage_record` 的 `event_id` 幂等键建议在 model 阶段就补（否则重复计量、billing 阶段再补要回改）。④ **另需开发者裁决**：阶段 5 的「DataScope 行级拦截器」归属（建议不阻塞阶段 6、但 kb 前必须做，口径已写入 §七）。⑤ 环境干净：隔离实例已停、测试数据已清、IDEA 三服务未动；工作区仅剩另一对话的 `AGENTS.md`/`LEARNING.md`（未动）。
